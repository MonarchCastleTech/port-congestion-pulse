# -*- coding: utf-8 -*-
"""Port Congestion Pulse - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["aisstream.io", "Freightos", "World Bank PMI", "UN Comtrade"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Ports Monitored", "value": "500", "delta": "+5%"},
            {"label": "Congested", "value": "34", "delta": "+2"},
            {"label": "Avg Wait (hrs)", "value": "18.5", "delta": "-3%"},
            {"label": "Disruptions (7d)", "value": "12", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "aisstream.io"}
            for i in range(1, 16)
        ],
        "timeseries": [
